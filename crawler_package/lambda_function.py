...
class AWSBlogCrawler:
    ...
    def extract_post_links_from_rss(self):
        """Extract post links from the blog's RSS feed.
        
        The AWS blog listing page renders links via JavaScript, which requests
        cannot execute. The RSS feed provides all post URLs reliably.
        """
        feed_url = self.base_url.rstrip('/') + '/feed/'
        print(f"Fetching RSS feed: {feed_url}")
        
        xml_text = self.get_page(feed_url)
        if not xml_text:
        ...
        try:
            ...
            # RSS 2.0 format: <rss><channel><item><link>...</link></item></channel></rss>
    ...
    def crawl_all_posts(self, max_pages=None):
        ...
        # Step 1: Try RSS feed first (reliable — not affected by JS rendering)
        rss_links = self.extract_post_links_from_rss()
        if rss_links:
            print(f"RSS feed returned {len(rss_links)} posts")
            for link in rss_links:
                if '/category/' not in link and '/tag/' not in link:
                    all_post_urls.add(link)
        
        # Step 2: Also scrape HTML listing pages for any posts not in RSS
...
class BuilderAWSCrawler:
            ...
            if content_changed:
                ...
                update_expression = f'''
                    SET #url = :url,
                        ...
                        label_confidence = :zero,
                        label_generated = :empty,
                        #source = :source,{filter_expression_parts}
                '''
                expression_values = {
                    ':url': metadata['url'],
                    ':title': metadata['title'],
                ...
                update_expression = f'''
                    SET #url = :url,
                        ...
                        content = if_not_exists(content, :content),{filter_expression_parts}
                '''
    ...
    def crawl_all_posts(self):
            ...
            try:
                ...
                for url_elem in root.findall('.//ns:url', namespace):
                    ...
                    if loc is not None and lastmod is not None:
                        ...
                        # Filter for EUC-related content using keyword filter
                        result = filter_post(url, title)
                        if result.accepted:
                            all_posts.append((url, date, result))
                            self.posts_accepted += 1
                        else:
                            # Log keyword rejection details
                            self.posts_keyword_rejected += 1
        ...
        # Process each post
        for idx, (url, lastmod, filter_result) in enumerate(all_posts, 1):
            print(f"[{idx}/{len(all_posts)}] Processing: {url}")
            
            metadata = self.extract_metadata_from_sitemap(url, lastmod)
            
            # Attach keyword filter metadata for DynamoDB storage
        ...
        print(f"  Posts keyword-accepted: {self.posts_accepted}")
        print(f"  Posts keyword-rejected: {self.posts_keyword_rejected}")
        print(f"{'='*60}")
        ...
        return {
            ...
            'posts_keyword_rejected': self.posts_keyword_rejected
        }
...
def lambda_handler(event, context):
    ...
    try:
        ...
        results = {
            ...
            'posts_accepted': sum(r.get('posts_accepted', 0) for r in all_results.values()),
            'posts_keyword_rejected': sum(r.get('posts_keyword_rejected', 0) for r in all_results.values()),
        ...
        if aws_blog_summaries_needed > 0:
            ...
            try:
                ...
                # Select the alias from the event-resolved environment (see line ~804),
                # NOT the crawler's own static ENVIRONMENT env var. The prod crawler is
                # pinned to ENVIRONMENT=production, so reading the env var here would make
                # a staging-triggered run invoke the :production alias (Issue 3).
        ...
        if aws_blog_classification_needed > 0:
            ...
            try:
                ...
                # Select the alias from the event-resolved environment (see line ~804),
                # NOT the crawler's own static ENVIRONMENT env var (Issue 3). The :staging
                # alias is pinned to ENVIRONMENT=staging and therefore the staging table,
                # so picking the right alias is what actually routes the classifier.
                ...
                for i in range(num_batches):
                    lambda_client.invoke(
                        ...
                        Payload=json.dumps({
                            'batch_size': batch_size,
                            # Match the summary payload so the classifier can route by
                            # event too once it honors these (defense in depth on top of
                            # the alias selection above).
        ...
        print(f"  EUC Filter: {results.get('posts_accepted', 0)} keyword-accepted, "
              f"{results.get('posts_keyword_rejected', 0)} keyword-rejected")
