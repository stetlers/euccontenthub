...
class ChatWidget {
        ...
        if (role === 'assistant') {
            proposalSuggestionHTML = `
                <div class="chat-proposal-suggestion">
                    <div class="chat-proposal-text">ðŸ’¡ Can't find what you're looking for?</div>
                    <button class="chat-proposal-btn" onclick="document.getElementById('proposeArticleBtn').click(); document.getElementById('chatCloseBtn').click();">
                        âœï¸ Propose a Community Article
                    </button>
                </div>
            `;
        }
    ...
    createRecommendationHTML(rec) {
        ...
        return `
            <div class="chat-recommendation" onclick="window.open('${rec.url}', '_blank')">
                ...
                <div class="chat-recommendation-reason">ðŸ’¡ ${this.escapeHtml(rec.relevance_reason)}</div>
    ...
    getLabelIcon(label) {
        const iconMap = {
            ...
            'Customer Story': 'ðŸ¢',
            'Technical How-To': 'ðŸ”§',
            'Thought Leadership': 'ðŸ’¡'
        };
        return iconMap[label] || 'ðŸ·ï¸';
    }
