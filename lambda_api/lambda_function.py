...
KB_DOCUMENTS = {
    ...
    'curated-qa/common-questions.md': {
        ...
        'description': 'Frequently asked questions about AWS WorkSpaces services',
    ...
    'service-mappings/service-renames.md': {
        ...
        'description': 'Complete history of AWS WorkSpaces service name changes',
...
SEARCH_ANALYTICS_LOW_COVERAGE_THRESHOLD = 3
...
def normalize_search_query(q):
    ...
    """Lowercase, trim, and collapse whitespace for dedup keying."""

    if not q:

        return ''

    return _SEARCH_WS_RE.sub(' ', q.strip().lower())



def _extract_user_alias(event):

    """Best-effort alias extraction from JWT in Authorization header. Returns None on failure."""
    ...
    try:
        ...
        return decoded.get('email') or decoded.get('cognito:username') or decoded.get('sub')
...
def log_search_query(raw_query, result_count, user_alias):

    """Aggregated counter update for search analytics. Swallows all errors (non-blocking)."""
    ...
    try:
        ...
        ttl = int(time.time()) + SEARCH_ANALYTICS_RETENTION_DAYS * 86400

        is_zero = result_count == 0

        is_low = result_count < SEARCH_ANALYTICS_LOW_COVERAGE_THRESHOLD

        users_to_add = {user_alias} if user_alias else None



        update_expr = "ADD #count :one"

        if users_to_add:

            update_expr += " , users_seen :users"

        update_expr += (

            " SET last_query_raw = :raw, last_result_count = :rc, "

            "last_seen_at = :now, last_user_alias = :ua, "

            "flag_zero_results = :zero, flag_low_coverage = :low, "
        ...
        expr_values = {
            ...
            ':raw': raw_query[:500] if raw_query else '',

            ':rc': result_count,

            ':now': now,

            ':ua': user_alias or 'anonymous',
        ...
        if users_to_add:
            ...
            expr_values[':users'] = users_to_add



        search_analytics_table.update_item(

            Key={'normalized_query': normalized},
        ...
        print(f"[search-analytics] log failed (non-fatal): {e}")
...
# JWKS cache: Cognito rotates signing keys rarely, so cache for an hour. A kid
# miss forces an immediate refresh (handles rotation between cache windows), but
# forced refreshes are rate-limited so a flood of tokens bearing bogus kids
# cannot amplify into one outbound JWKS fetch per request.
_JWKS_CACHE = {'keys': None, 'fetched_at': 0, 'last_forced': 0}
_JWKS_TTL_SECONDS = 3600
_JWKS_MIN_FORCED_REFRESH_SECONDS = 60

# DER-encoded DigestInfo prefix for SHA-256, per RFC 8017 (PKCS#1 v1.5).
_SHA256_DIGEST_INFO = bytes.fromhex('3031300d060960864801650304020105000420')

# Expected token issuer for this user pool.
_EXPECTED_ISSUER = f'https://cognito-idp.{COGNITO_REGION}.amazonaws.com/{COGNITO_USER_POOL_ID}'

# Allowable clock skew (seconds) between this Lambda and Cognito when checking
# time-based claims (exp / nbf). A small window tolerates minor drift without
# meaningfully extending a token's usable lifetime.
_CLOCK_SKEW_LEEWAY_SECONDS = 60


def _b64url_decode(segment):
    """Decode a base64url JWT segment, restoring padding."""
    return base64.urlsafe_b64decode(segment + '=' * (-len(segment) % 4))


def _get_jwks(force_refresh=False):
    """Return Cognito's JWKS key list, cached with a TTL.

    A forced refresh (on a kid miss) is itself rate-limited: if we refreshed
    within the last _JWKS_MIN_FORCED_REFRESH_SECONDS we serve the cached keys
    instead, so unknown-kid floods can't drive one outbound fetch per request.
    The normal TTL refresh is unaffected.
    """
    now = time.time()
    forced = force_refresh and now - _JWKS_CACHE['last_forced'] >= _JWKS_MIN_FORCED_REFRESH_SECONDS
    if forced or _JWKS_CACHE['keys'] is None or now - _JWKS_CACHE['fetched_at'] > _JWKS_TTL_SECONDS:
        if force_refresh:
            _JWKS_CACHE['last_forced'] = now
        with _urllib_request.urlopen(JWKS_URL, timeout=5) as resp:
            _JWKS_CACHE['keys'] = json.loads(resp.read().decode('utf-8')).get('keys', [])
            _JWKS_CACHE['fetched_at'] = now
    return _JWKS_CACHE['keys']


def _find_jwk(kid):
    """Find the JWK for a given key id, refreshing once on a miss (key rotation).

    The forced refresh is throttled inside _get_jwks, so a miss does not
    guarantee a network call.
    """
    for jwk in _get_jwks():
        if jwk.get('kid') == kid:
            return jwk
    for jwk in _get_jwks(force_refresh=True):
        if jwk.get('kid') == kid:
            return jwk
    return None
...
def _rsa_pkcs1_v15_verify(n, e, message, signature):
    """Verify an RSASSA-PKCS1-v1_5 / SHA-256 signature with no third-party deps.

    Recovers the padded message from the signature (s^e mod n) and compares it,
    in constant time, against the expected EMSA-PKCS1-v1_5 encoding of SHA-256.
    """
    k = (n.bit_length() + 7) // 8
    if len(signature) != k:
        return False
    ...
    ps_len = k - len(der) - 3
    if ps_len < 8:  # malformed / too-short modulus
        return False
...
def validate_jwt_token(token):
    """
    Validate a Cognito JWT: verify the RS256 signature against the pool's JWKS,
    then check issuer, expiry, audience, and token_use. Raises on any failure.

    Signature verification is done with the standard library only (no PyJWT /
    cryptography dependency), so it runs in the Lambda as-is.
    """
    try:
        ...
        # --- Signature verification (the part that was previously skipped) ---
        if header.get('alg') != 'RS256':
            raise Exception('Unsupported token algorithm')
        ...
        if not _rsa_pkcs1_v15_verify(n, e, signing_input, signature):
            raise Exception('Invalid token signature')

        # --- Claim checks ---
        if decoded.get('iss') != _EXPECTED_ISSUER:
            raise Exception('Invalid issuer')

        now = time.time()

        # Expiry, with a small leeway for clock skew between us and Cognito.
        if decoded.get('exp', 0) < now - _CLOCK_SKEW_LEEWAY_SECONDS:
            raise Exception('Token has expired')

        # Not-before, if present: reject tokens not yet valid (same leeway).
        nbf = decoded.get('nbf')
        if nbf is not None and nbf > now + _CLOCK_SKEW_LEEWAY_SECONDS:
...
def moderate_comment(text, post_context):
    ...
    def call_bedrock():
        ...
        try:
            ...
            prompt = f"""You are a content moderator for the Amazon WorkSpaces Community Hub, a technical community platform. Analyze the following comment and determine if it should be approved or flagged for review.
...
1. SPAM/PROMOTIONAL (Flag if):
   ...
   - Promotes products/services unrelated to AWS WorkSpaces
...
4. OFF-TOPIC (Flag if):
   ...
   - Discusses topics completely unrelated to AWS, cloud, or WorkSpaces
...
- Personal experiences with AWS WorkSpaces services
...
def lambda_handler(event, context):
    ...
    global search_analytics_table, SEARCH_ANALYTICS_TABLE_NAME
    ...
    # Initialize search analytics table

    SEARCH_ANALYTICS_TABLE_NAME = f'search-analytics{table_suffix}'

    search_analytics_table = dynamodb.Table(SEARCH_ANALYTICS_TABLE_NAME)
    ...
    try:
        ...
        if path == '/posts' and http_method == 'GET':
            ...
            return get_all_posts(query_parameters, event)

        elif path == '/search-analytics/gaps' and http_method == 'GET':

            return get_search_gaps(event, query_parameters)
        ...
        elif path.startswith('/chat/history/') and http_method == 'PUT':

            conversation_id = path.split('/')[-1]
            body = json.loads(event.get('body', '{}'))
            return save_chat_conversation(event, conversation_id, body)
...
def get_all_posts(query_params, event=None):
    ...
    try:
        ...
        if search_query:
            ...
            log_search_query(search_query, len(items), _extract_user_alias(event))
...
def get_search_gaps(event, query_params):

    """Return top no-result / low-coverage queries by frequency."""

    try:

        limit = min(int(query_params.get('limit', 50)), 500)

        only_zero = str(query_params.get('zero_only', '')).lower() == 'true'

        only_low = str(query_params.get('low_only', '')).lower() == 'true'

        min_count = int(query_params.get('min_count', 1))



        items = []

        response = search_analytics_table.scan()
        ...
        if only_zero:
            ...
            items = [i for i in items if i.get('flag_zero_results')]

        elif only_low:

            items = [i for i in items if i.get('flag_low_coverage')]

        items = [i for i in items if int(i.get('count', 0)) >= min_count]
        ...
        for i in items:
            ...
            users_seen = i.get('users_seen') or []

            i['unique_user_count'] = len(users_seen)

            i.pop('users_seen', None)
        ...
        return {
            ...
            'body': json.dumps({'gaps': items, 'count': len(items)}, cls=DecimalEncoder),

        }

    except Exception as e:

        print(f"Error in get_search_gaps: {str(e)}")
...
def trigger_crawler(event):
    ...
    try:
        ...
        # [REMOVED 2026-06-24] The broken Playwright/zip builder crawler invoke was
        # deleted here. It scraped the unrendered SPA shell and wrote
        # 'AWS Builder Center' as title/content (the prod incident). Builder
        # content is now produced by the reliable ECS selenium crawler
        # (selenium-crawler-cluster / selenium-crawler-task), driven by the
        # weekly schedule and by aws-blog-crawler's ECS delegation.
        # builder_crawler_fn invoke intentionally omitted.
        ...
        try:
            ...
            if auth_header:
                ...
                try:
                    ...
                    # Optional auth: a bad/absent token just means anonymous.
                    # We make no authz decision on current_user_id here beyond
                    # gating the caller's own pending comments, so failing to
                    # anonymous is safe. Log for visibility, then continue.
                    print(f"[auth] optional JWT validation failed, continuing anonymous: {_auth_err}")

        except Exception as _hdr_err:

            print(f"[auth] could not read auth header, continuing anonymous: {_hdr_err}")
...
@require_auth
def save_chat_conversation(event, conversation_id, body):
    """Save or update a chat conversation."""
    user_id = event['user']['sub']
    now = get_timestamp()
    
    try:
        title = body.get('title', 'Conversation')[:100]
        ...
        return {
            'statusCode': 200,
            'headers': cors_headers(),
            'body': json.dumps({'message': 'Conversation saved'})
        }
    except Exception as e:
        ...
        try:
            ...
            if auth_header:
                ...
                try:
                    ...
                    # Optional auth: failing to anonymous is safe here (the only
                    # use of current_user_id is gating the caller's own pending
                    # comments). Log for visibility, then continue anonymous.
                    print(f"[auth] optional JWT validation failed, continuing anonymous: {_auth_err}")
...
KB_DOCUMENTS = [

    {
        ...
        'name': 'WorkSpaces Community Q&A Pairs',

        'description': 'Common questions and answers about AWS WorkSpaces services',
...

    {
        ...
        'name': 'WorkSpaces Service Mappings',

        'description': 'Mappings between WorkSpaces services and AWS services',
...
def refine_article_proposal(user_input, selected_category=None):
    ...
    prompt = f"""You are an expert AWS technical writer helping someone create a community article proposal for the Amazon WorkSpaces Community Hub.
...
EUC_SERVICES = [
    ...
    "Amazon WorkSpaces Thin Client", "Amazon DCV", "Other/General WorkSpaces"
...
def refine_feature_proposal(service, title, description, priority, use_case=None):
    ...
    try:
        ...
        prompt = f"""You are an Amazon WorkSpaces Community product manager. A community member has submitted a feature request. Analyze and enhance it.
...
def refine_innovation_to_feature(innovation):
    ...
    try:
        ...
        prompt = f"""You are an Amazon WorkSpaces Community product manager. A community member has developed an innovation that reveals a potential service feature opportunity. Analyze and transform it into a feature request.
