...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadWhatsNewChiron(); // Load What's New ticker
...
async function loadWhatsNewChiron() {
    ...
    try {
        ...
        const itemsHtml = announcements.map(a => {
            ...
            const tag = (a.service_tag || '').replace(/</g, '&lt;').replace(/>/g, '&gt;');
            const url = (a.url || '#').replace(/"/g, '&quot;');
            const date = a.date_published ? new Date(a.date_published).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '';
        ...
        // Remove CSS animation — we'll use JS for scrolling
        ...
        function scrollTicker() {
            if (!paused) {
                ...
                if (scrollPos >= halfWidth) {
                    scrollPos -= halfWidth; // seamless loop
                }
    ...
    try {
        ...
        const counts = months.map(m => {
            return announcements.filter(a => {
                ...
                // Try YYYY-MM prefix match
                if (dp.startsWith(m)) return true;
                // Try parsing the date and comparing year-month
                try {
                    const d = new Date(dp);
                    if (!isNaN(d)) {
...
async function loadKBLeaderboard() {
    ...
    try {
        const response = await fetch(`${API_ENDPOINT}/kb-contributors?limit=10`);
        if (!response.ok) throw new Error('API error');

        const data = await response.json();
        const contributors = data.contributors || data.leaderboard || data.top_contributors || [];
...
class ProposalsReview {
    ...
    openModal() {
        this.currentUserId = window.authManager && window.authManager.isAuthenticated()
            ? window.authManager.getUser()?.sub : null;
        this.modal.style.display = 'flex';
        document.body.style.overflow = 'hidden';
        this.fetchProposals();
    }
    ...
    renderProposals() {
        const listEl = this.modal.querySelector('#reviewProposalsList');
        if (this.proposals.length === 0) { listEl.innerHTML = '<div class="proposals-empty">No proposals found.</div>'; return; }
        ...
        listEl.querySelectorAll('.proposal-card').forEach(card => {
            card.addEventListener('click', (e) => {
                ...
                this.toggleExpand(card.dataset.proposalId);
            });
        });
        listEl.querySelectorAll('.proposal-feedback-btn').forEach(btn => {
            btn.addEventListener('click', (e) => { e.stopPropagation(); this.expandedProposalId = btn.dataset.proposalId; this.renderProposals(); });
        });
        listEl.querySelectorAll('.proposal-upvote-btn, .proposal-downvote-btn').forEach(btn => {
            btn.addEventListener('click', (e) => { e.stopPropagation(); this.handleVote(btn.dataset.proposalId, btn, btn.dataset.voteType); });
        });
        listEl.querySelectorAll('.proposal-comment-submit').forEach(btn => {
            btn.addEventListener('click', (e) => { e.stopPropagation(); this.handleComment(btn.dataset.proposalId); });
        });
        listEl.querySelectorAll('.proposal-delete-btn, .proposal-delete-btn-inline').forEach(btn => {
            btn.addEventListener('click', (e) => { e.stopPropagation(); this.handleDelete(btn.dataset.proposalId); });
        });
    }
    ...
    createProposalCard(proposal) {
        ...
        if (isExpanded) {
            ...
            if (isFeature) {
                detailsHtml = `
                    <div class="proposal-full-description"><h4>Full Description</h4><p>${escapeHtml(description)}</p></div>
                ...
                <div class="proposal-expanded-details">
                    ...
                    <div class="proposal-comments-section">
                        ...
                        <div class="proposal-comment-form">
                            <textarea class="proposal-comment-input" data-proposal-id="${proposal.proposal_id}" placeholder="Share your feedback..." rows="2" maxlength="500"></textarea>
            ...
            <div class="proposal-card ${isExpanded ? 'proposal-card-expanded' : ''}" data-proposal-id="${proposal.proposal_id}">
                ...
                <div class="proposal-card-footer">
                    ...
                    <div class="proposal-card-actions">
                        ${isOwner ? `<button class="proposal-delete-btn-inline" data-proposal-id="${proposal.proposal_id}" title="Delete">🗑️</button>` : ''}
                        <button class="proposal-feedback-btn" data-proposal-id="${proposal.proposal_id}" title="Feedback">💬 <span>${commentCount}</span></button>
                        <button class="proposal-upvote-btn ${hasVoted ? 'already-voted' : ''}" data-proposal-id="${proposal.proposal_id}" data-vote-type="up" ${hasVoted ? 'disabled' : ''} title="Upvote">👍 <span class="proposal-vote-count">${upvotes}</span></button>
                        <button class="proposal-downvote-btn ${hasVoted ? 'already-voted' : ''}" data-proposal-id="${proposal.proposal_id}" data-vote-type="down" ${hasVoted ? 'disabled' : ''} title="Downvote">👎 <span class="proposal-vote-count">${downvotes}</span></button>
                    </div>
                </div>
    ...
    toggleExpand(proposalId) { this.expandedProposalId = this.expandedProposalId === proposalId ? null : proposalId; this.renderProposals(); }

    async handleVote(proposalId, btn, voteType) {
        if (!window.authManager || !window.authManager.isAuthenticated()) { showNotification('Please sign in to vote', 'info'); return; }
        ...
        try {
            ...
            this.renderProposals();
            const statusMsg = data.proposal.status !== 'pending' ? ` Proposal is now ${data.proposal.status}!` : '';
            showNotification(`Vote recorded!${statusMsg}`, 'success');
        } catch (error) { console.error('Error voting:', error); showNotification(`Error voting: ${error.message}`, 'error'); }
    }

    async handleComment(proposalId) {
        if (!window.authManager || !window.authManager.isAuthenticated()) { showNotification('Please sign in to leave feedback', 'info'); return; }
        const textarea = this.modal.querySelector(`.proposal-comment-input[data-proposal-id="${proposalId}"]`);
        const text = textarea ? textarea.value.trim() : '';
        if (!text) { showNotification('Please enter your feedback', 'error'); return; }
        try {
            const token = window.authManager.getIdToken();
            ...
            const data = await response.json();
            const idx = this.proposals.findIndex(p => p.proposal_id === proposalId);
            if (idx !== -1) { if (!this.proposals[idx].comments) this.proposals[idx].comments = []; this.proposals[idx].comments.push(data.comment); }
            this.renderProposals();
            showNotification('Feedback posted!', 'success');
        } catch (error) { console.error('Error posting feedback:', error); showNotification(`Error: ${error.message}`, 'error'); }
    }
    ...
    async handleDelete(proposalId) {
        ...
        } catch (error) { console.error('Error deleting:', error); showNotification(`Error: ${error.message}`, 'error'); }
    }
}
            ...
            options: {
                ...
                onClick: (event, activeElements) => {
                    if (activeElements.length > 0) {
                        ...
                        this.close();
                        setTimeout(() => {
                            // Filter posts to this week's date range
...
class InnovationHub {
    ...
    openDetail(innovation) {
        ...
        content.querySelector('#innovationViewProposal').addEventListener('click', (e) => {
            ...
            showNotification('Opening proposal...', 'info');
            // Navigate to proposals section — the ArticleProposal class handles display
            if (window.articleProposal) { window.articleProposal.showBrowsePanel(); }
        });
...
class InteractiveTour {
    ...
    static FEATURES = [
        ...
        {
            id: 13, num: '13', icon: '💡', name: 'Innovation Hub', tag: 'innovation', color: '#10b981',
    ...
    static HOTSPOT_POSITIONS = {
        ...
        13: { x: 70, y: 3 }    // Innovation Hub — header nav area
    };
        ...
        for (const f of InteractiveTour.FEATURES) {
            ...
            items += `
                <button class="tour-sidebar-btn" data-action="sidebarClick" data-feature-id="${f.id}"
                    ...
                    <span class="tour-sidebar-btn-name" style="color:${nameColor};font-weight:${nameWeight};">${f.name}</span>
                    ${indicator}
                </button>
            `;
        }
        ...
        return `
            <div class="tour-detail">
                <div class="tour-detail-content">
                    ...
                    <div style="font-size:20px;font-weight:800;color:#f1f5f9;line-height:1.2;margin-bottom:6px;">${f.name}</div>
    ...
    clickHotspot(featureId) {
        if (featureId < 1 || featureId > 12) return;
        if (this.tourStep >= 0) {
